...
/* =========================================================
   ...
   Simple Reward / Claim / Auto Stake
...
function getWalletProvider() {
  /*
    Bitget Wallet Mobile / Extension
  */
  ...
  */
  ...
  /*
    MetaMask หรือ Wallet Provider ทั่วไป
  */
...
function getAddress(...keys) {
  for (const key of keys) {
    ...
    if (
      window.APP_CONFIG?.contracts?.[key]
    ) {
...
async function ensureBSCNetwork() {
  ...
  if (!walletProvider) {
    throw new Error(
      "กรุณาเปิดเว็บไซต์ผ่าน Bitget Wallet หรือ DApp Browser"
    );
  }
...
async function requestCurrentAccount() {
  ...
  /*
    ขอ Account จาก Wallet ที่เปิดอยู่ในขณะนี้
  */
...
async function connectWallet() {
  try {
    ...
    */
    ...
    /*
      ตรวจและเปลี่ยนเป็น BSC
    */

    await ensureBSCNetwork();

    /*
      ขอ Account ปัจจุบันจาก Wallet
    */

    const selectedAddress =
      await requestCurrentAccount();

    /*
      สร้าง ethers Provider จาก Bitget/Wallet
    */
    ...
    /*
      บังคับเลือก Signer ของ Address
      ที่ Wallet ส่งกลับมา
    */
    ...
    /*
      ตรวจว่า Signer และ Account ตรงกัน
    */
    ...
    if (
      ...
      throw new Error(
        "บัญชีที่ Wallet เลือกไม่ตรงกับ Signer"
      );
    }
...
async function autoConnect() {
  /*
    ไม่ดึงบัญชีเก่าอัตโนมัติ
    ผู้ใช้ต้องกด Connect Wallet ทุกครั้ง
  */
  ...
  if (!walletProvider) {
    setStatus(
      "กรุณาเปิดผ่าน Bitget Wallet หรือ DApp Browser",
      "error"
    );
