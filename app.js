...
function findWalletStatusElement() {
  ...
  const elementByAttribute = document.querySelector(
...
async function ensureBSCNetwork() {
  if (!window.ethereum) {
    throw new Error(
      "กรุณาติดตั้ง MetaMask หรือเปิดผ่าน DApp Browser"
    );
  }
...
async function connectWallet() {
  try {
    ...
    const connectButton = findElement(
      "connectWalletBtn",
      "connectBtn",
      "btnConnect"
    );
...
async function loadRewardInfo() {
  ...
  await updateRewardProgress(orgVolume);
}
...
async function updateRewardProgress(orgVolume) {
  ...
  try {
    target =
      await rewardCore.requiredOrganizationVolume();
  } catch (error) {
    console.warn(
  ...
  const currentNumber = Number(
    ethers.formatUnits(orgVolume, 18)
  );
...
async function loadNCBalance() {
  if (!ncToken || !userAddress) {
    return;
  }

  const balance =
    await ncToken.balanceOf(userAddress);
...
async function loadStakeSummary() {
  ...
  if (withdrawMaturedButton) {
    withdrawMaturedButton.disabled =
      withdrawable === 0n;
  }
}
...
function updateCountdowns() {
  ...
  countdownElements.forEach((element) => {
    ...
    if (remaining <= 0) {
      ...
      const lotCard =
        element.closest(".stake-lot");

      const existingButton =
        lotCard?.querySelector(
          ".withdraw-lot-btn"
        );
